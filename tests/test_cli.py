...
def test_check_policy_on_default_policy_prints_effective_policy(capsys):
    exit_code = main(["check-policy", "--config", "policies/default.yaml"])
...
def test_check_policy_probe_explains_a_denial(capsys):
    exit_code = main([
        "check-policy", "--config", "policies/default.yaml",
...
def test_check_policy_probe_with_tools_json_classifies_by_schema(capsys):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        exit_code = main([
            "check-policy", "--config", "policies/default.yaml", "--tools", tools_path,
...
def test_check_policy_probe_rejects_bad_json(capsys):
    with pytest.raises(SystemExit) as exc_info:
        main(["check-policy", "--config", "policies/default.yaml", "--probe", "t", "not json"])
    assert exc_info.value.code == 2
